...
const markdownButtons = new Map();
const unicodeButtons = new Map();
const activeMarkdownStyles = new Set();
let activeUnicodeStyle = null;
let lastEditorValue = '';
let isApplyingUnicodeInput = false;
...
function updateUnicodeActiveStates() {
    unicodeButtons.forEach((btn, name) => {
        btn.classList.toggle('active', activeUnicodeStyle === name);
    });
}

function handleEditorInput(event) {
    const editor = event.target;
    if (isApplyingUnicodeInput) {
        lastEditorValue = editor.value;
        updatePreview();
        return;
    }
    if (activeUnicodeStyle) {
        const data = unicodeFonts[activeUnicodeStyle];
        if (data) {
            const diff = editor.value.length - lastEditorValue.length;
...
function applyUnicodeStyle(name) {
    ...
    if (activeUnicodeStyle === name) {
        activeUnicodeStyle = null;
        updateUnicodeActiveStates();
        showNotification(translations[currentLang]['style-applied'], 'success');
        return;
    }
    activeUnicodeStyle = name;
    updateUnicodeActiveStates();
    showNotification(translations[currentLang]['style-applied'], 'success');
}
